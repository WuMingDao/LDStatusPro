...
|------|----------|----------------|--------------|
| 日榜 | 当天 00:00 至今 | 每 10 分钟（Cron 定时） | 5 分钟 |
| 周榜 | 本周一至今 | 每天 08:00（Cron 定时） | 24 小时 |
| 月榜 | 本月1日至今 | 每天 08:00（Cron 定时） | 24 小时 |

### 服务器端计算机制

排行榜采用 **Cron Triggers 定时预计算** 机制：

1. **日榜**：每 10 分钟由 Cloudflare Cron 自动触发计算，结果存入 KV 缓存
2. **周榜/月榜**：每天 UTC 00:00（北京时间 08:00）自动计算
...
**A:**
- 服务器端：日榜每 5 分钟计算一次，周榜/月榜每天计算一次
- 客户端：日榜可每 1 分钟刷新一次，周榜/月榜可每天刷新一次
- 客户端不会自动刷新，需要点击对应标签才会尝试获取新数据

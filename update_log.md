...
- 📢 **系统公告栏** - 支持管理员发布滚动公告
  ...
  - 公告始终显示，用户无法关闭
  - 滚动速度根据文字长度自动调整（10-30秒）
  - 新增 API：`GET /api/config/announcement`（公开）、`GET/PUT /api/admin/config/announcement`
  - 管理面板「系统设置」新增公告配置区域（内容、类型、有效期）
...
- 🧹 **代码结构优化** - 添加 `Utils.hash()` 用于公告内容指纹计算

**数据库变更：**
- 新增 `system_config` 表用于存储公告等配置
- 迁移脚本：`scripts/migrate-add-system-config.sql`

**云同步性能深度优化（参考 reading 同步逻辑）：**
- ✨ **增量同步优先** - 新增 `syncTodayRequirements()` 方法
  - 只同步当天的升级要求数据，调用 `/api/requirements/sync` 接口
  - 同步间隔：1小时
  - 数据量：单条记录 vs 全部历史数据
- 🔄 **条件全量同步** - `uploadRequirementsFull()` 仅在首次登录或本地数据天数多于云端时触发
  - 同步间隔：12小时
  - 减少 requirements sync-full 请求约 **90%**
